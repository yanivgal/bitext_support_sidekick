...
def _get_unstructured_prompt() -> str:
    ...
    return (
        ...
        "If the user asks 'What do you think I should ask next?' or 'Advise me what to query next' or similar:\n"
        "- Have a thoughtful conversation about what they might want to explore\n"
        "- Analyze their interests, past queries, and conversation context deeply\n"
        "- Give 1-2 specific, thoughtful suggestions with detailed reasoning\n"
        "- Explain why these suggestions would be valuable based on their interests\n"
        "- Be conversational and ask follow-up questions to understand their preferences\n"
        "- If they seem ready for specific recommendations, suggest they ask for 'clickable suggestions'\n"
        "- Focus on quality over quantity - better to have 1-2 thoughtful suggestions than 5 generic ones\n"
...
def unstructured_agent_node(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        resp = _get_llm().chat(llm_messages, tools_json=TOOLS_SCHEMA)
        ...
        if msg.tool_calls:
            ...
            final_resp = _get_llm().chat(final_messages, response_format=FinalResponse)
            ...
            final_resp = _get_llm().chat(llm_messages, response_format=FinalResponse)
            final_response = final_resp.choices[0].message.parsed
